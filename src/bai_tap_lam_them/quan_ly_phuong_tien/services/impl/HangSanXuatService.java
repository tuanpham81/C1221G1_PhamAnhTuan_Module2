...
//    }
    ...
    public void hienThi() {
        List<HangSanXuat> DataFromFile = ReadAndWriteHang.readDataFromFile(BRAND_FILE_DAT);
        if (DataFromFile.size()>0) {
            for (HangSanXuat hangSanXuat : DataFromFile) {
                System.out.println(hangSanXuats);
            }
        } else {
    ...
    public static HangSanXuat getHangSanXuat(String maHang) {
        ...
        for (int i = 0; i < DataFromFile.size(); i++) {
            if (DataFromFile.get(i).getMaHang().equals(maHang)) {
                return DataFromFile.get(i);
            }
        }
        return DataFromFile.get(0); // giá trị mặc định
    }
}
