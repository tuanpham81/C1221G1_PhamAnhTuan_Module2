...
import bai_tap_lam_them.quan_ly_phuong_tien.models.PhuongTien;
...
public class PhuongTienService{
    ...
    static ArrayList<PhuongTien> danhSachPhuongTien = new ArrayList<PhuongTien>();
    private static final String VEHICLES_FILE_DAT = "src\\bai_tap_lam_them\\quan_ly_phuong_tien\\data\\phuong_tien.csv";

    public static void hienThi() {
            for (int i = 0; i < danhSachPhuongTien.size(); i++) {
                System.out.println(danhSachPhuongTien.get(i));
        }
    }

    public static void xoa(String bienSoXe) {
        ...
        for (int i = 0; i < danhSachXeMay.size(); i++) {
            if (danhSachXeMay.get(i).getBienSo().equals(bienSoXe)) {
                ...
                do {
                    ...
                    if (choose2 == 1) {
                        danhSachOto.remove(i);
                        ReadAndWriteXeMay.writeToFile(XeMayService.BIKE_FILE_DAT,);
